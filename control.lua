--[[
 TODO:
 все еще баг с удалением https://i.gyazo.com/bcbb42fc257fd3e15fc8d3e9b4e1a813.png , предположительно после удаления ресивера жидкостного тессеракта. Есть какоето нотис на серной кислоте
...
function string.split(str,divider)
	if divider=='' then return str end
	local pos = 0
	local arr = {}
	...
	if id then
		...
		if global.tesseract.data[tostring(id)] then
			...
			for idx, data in pairs(global.tesseract.data) do
				if data.bufferId == id then -- изем ид через буффер который пришел в эвенте
		...
		if global.tesseract.senders[removeId] then -- ищем и удаляем из сендеров, если он есть
			global.tesseract.senders[removeId] = nil
		end
		for i, qid in ipairs(global.tesseract.queue) do -- ищем и удаляем из очереди, если он есть
			if tostring(qid) == removeId then
				table.remove(global.tesseract.queue, i)
				break
			end
		end
		for i, data in ipairs(global.tesseract.receivers) do -- ищем и удаляем из ресиверов, если он есть
...
local function loadGuiData(pid, entity)
	...
	if worktype and worktype == 2 then -- не прерываем загрузку, т.к. должны записать переключатель
...
local function onNthTickCheckQueue() -- проверка всех телепортов отмеченных как передатчик и добавление их в очередь
...
local function onNthTickSend()
	...
	for _, receiverData in ipairs(receivers) do -- прогоняем все ресиверы пока не найдем подходящий
		...
		if receiver and receiver.valid then
			...
			if receiverType == senderType then -- если оба жидкостные или оба вещевые
				if senderType then --is fluid
					local senderFluidCount, senderCapacity, senderFluidName = getFluidCount(sender)
					if senderFluidCount ~= 0 and senderFluidName then --если есть что отправлять
						...
						then
							if receiverCapacity > receiverFluidCount then -- и в ресивере еще осталось место
					...
					if senderItemCount ~= 0 then  --если есть что отправлять
						local receiverItemCount, receiverCapacity = getInventoryCount(receiver)
						if receiverCapacity > receiverItemCount then -- в ресивере еще есть место
							...
							for name, count in pairs(content) do
								...
								local insertCount = 0
								if insertStackCount <= freeCount then -- если отправляемых предметов меньше чем свободного места в ресивере
